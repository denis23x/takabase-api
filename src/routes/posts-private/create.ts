          ...
          await request.server.prisma.$transaction(async (prismaClient: PrismaClient): Promise<PostPrivate> => {
            ...
            if (postPrivateImage) {
              ...
              requestRollback.postPrivateImageList = async (): Promise<void> => {
                await request.server.storagePlugin.setImageListMove(postPrivateImageList, tempImageListDestination.shift());
              };

              // Rewrite the image URL in the request body with the new post image URL
              request.body.image = request.server.markdownPlugin.getImageListReplace(postPrivateImage, tempImageList, postPrivateImageList);
            }
            ...
            if (tempMarkdownList.length) {
              ...
              requestRollback.postPrivateMarkdownList = async (): Promise<void> => {
                await request.server.storagePlugin.setImageListMove(postPrivateMarkdownList, tempMarkdownListDestination.shift());
              };
