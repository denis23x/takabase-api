      ...
      while (requestRetries < MAX_RETRIES) {
        try {
          ...
          await request.server.prisma.$transaction(async (prismaClient: PrismaClient): Promise<Post> => {
            ...
            const postMarkdownImageList: string[] = request.server.markdownPlugin.getImageListFromBucket(request.server.markdownPlugin.getImageListFromBody(post.markdown));
